...
CREATE PROCEDURE [ETL_TEMP].[CustomerBasicDetail_Temp_A]  
AS
BEGIN
...
/*03112023 AMAAR - *SELECT CustomerId,(@CustomerEntityId + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) CustomerEntityId */
SELECT CustomerId, NEXT VALUE FOR [IBL_ENPA_DB_V2].[dbo].[Seq_CustomerEntityId] CustomerEntityId
	into #CustEntityID
...
/*03112023 AMAAR - SELECT UCIF_ID,(@UcifEntityID + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) UcifEntityID*/
SELECT UCIF_ID, NEXT VALUE FOR [IBL_ENPA_DB_V2].[dbo].[Seq_UcifEntityID] UcifEntityID
	INTO #UcifEntityID
